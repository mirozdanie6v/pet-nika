...
export function BookingFlow({onDone}:{onDone:()=>void}){
 ...
 return <div className="booking-card premium-booking paw-surface">
   ...
   {step===2&&<Choice title={c.choosePet}>{state.pets.map(p=><button key={p.id} className={`choice pet-choice ${petId===p.id?'selected':''}`} onClick={()=>setPetId(p.id)}><span className="choice-pet-photo"><img src={getPetPhoto(p)} alt={petValue(p.name)}/></span><span><b>{petValue(p.name)}</b><small>{petValue(p.breed)} · {petValue(p.age)}</small></span></button>)}</Choice>}
   ...
   {step===7&&<Choice title={c.review}><div className="booking-review-pet"><img src={getPetPhoto(pet)} alt={petValue(pet.name)}/><span><small>{c.pet}</small><b>{petValue(pet.name)}</b><em>{petValue(pet.breed)}</em></span></div><div className="summary"><Row a={c.service} b={service[state.language]}/><Row a={c.date} b={date}/><Row a={c.time} b={time}/></div></Choice>}
